...
// v.2.7 20130908
//(c)2002-2013 Websoft Ltd., Russia
...
var g_sAudioWarning = "Модуль содержит звуковые файлы. Нажмите кнопку, чтобы разрешить их воспроизведение.";
var g_sProceed = "Продолжить";
...
var clTouch =
{
	...
	End: function (oElement, oEvent) {
		...
		if (g_bDragOn) {
			...
			//GetDropTarget(oElement.getAttribute("touchx"), oElement.getAttribute("touchy"), oElement.id);
			...
			if (oTarget.style.display != "inline" && oTarget.style.display != "block") {
				...
				if (nTransition) {
					...
					if (oObject) {
						...
						if (nTransition != null) {
							if (g_isMSIE && (oTarget.filters != null)) {
...
function OpenFrame(oNewFrame, bKeepPreviousFrame) {
	...
	for (var i = 0; i < nObjects; i++) {
		...
		if (nTransition != null && nBegin == 0 && sDisplay != "none") {
			...
			if (g_isMSIE && (oElement.filters != null)) {
...
function OpenObject(oObject, bForceDisplay) {
	...
	if (oData == null) {
		if (g_isMSIE) {
			if (oObject.getAttribute("preload") != null && g_nVersion >= 7)  // PNG images
			{
		...
		if (oChild.id != id) {
			...
			if (oStyle.display != "none") {
				if (parseInt(oStyle.left) <= x && parseInt(oStyle.left) + parseInt(oStyle.width) > x &&
					...
					//alert(oChild.id + " " + oStyle.display + " " + x + "," + y);
...
function DisplayMask(bOn) {
	...
	if (bOn) {
		...
		if (oMask == null) {
			...
			if (g_isMSIE && oMask.filters != null) {
